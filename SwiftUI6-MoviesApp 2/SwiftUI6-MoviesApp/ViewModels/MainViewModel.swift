...
class MainViewModel: ObservableObject {
        ...
        NetworkService().getData(urlString: urlString) { (result: Result<Movies, Error>) in
            DispatchQueue.main.async {
                ...
                case .success(let movies):
                    print("Fetched movies successfully: \(movies.results.count) movies")
                    self.movies = movies.results
                case .failure(let error):
                    print("Failed to fetch movies with error: \(error)")
    ...
    func fetchGenres() {
        ...
        print("Fetching genres from URL: \(urlString)")
        NetworkService().getData(urlString: urlString) { (result: Result<Genres, Error>) in
            DispatchQueue.main.async {
                switch result {
                case .success(let genres):
                    print("Fetched genres successfully: \(genres.genres.count) genres")
                    self.genres = genres.genres
                case .failure(let error):
                    print("Failed to fetch genres with error: \(error)")
                    self.showFetchError = true
                }
            }
        }
